...
local function check_member(cb_extra, success, result)
   ...
   for k,v in pairs(result.members) do
      ...
      if member_id ~= our_id then
          ...
          return send_large_msg(receiver, 'شما به مقام خریدار گروه ارتقا داده شدید.')
      end
    end
end
...
local function automodadd(msg)
  ...
  if msg.action.type == 'chat_created' then
      ...
      save_data(_config.moderation.data, data)
      return 'گروه با موفقیت اضافه شد, و @'..username..' به درجه ادمین گروه ارتقا داده شد.'
   end
end
...
local function promote(receiver, member_username, member_id)
  ...
  if data[group]['moderators'][tostring(member_id)] then
    ...
    save_data(_config.moderation.data, data)
    return send_large_msg(receiver, '@'..member_username..' به ادمین های گروه اضافه شد.')
end
